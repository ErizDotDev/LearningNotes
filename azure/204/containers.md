...
- Deploying applications in the same container app environment is useful:
	- For hosting related applications and services
	- For deploying different applications in the same virtual network
	- For hosting Dapr applications that communicate via Dapr service communication API
	- For grouping up applications that share the same Dapr configuration
	- For grouping up applications that share the same Log Analytics workspace

- Consider deploying applications in different container app environments when:
	- Applications will never share the same compute resource
	- Dapr application don't use the service communication API
